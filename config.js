...
function buildEntertainerSettings(baseSettings, overrides = {}) {
    ...
    const settings = {
        ...
        roomHealthIntervalMs: Math.max(10000, normalizeInt(merged.roomHealthIntervalMs, 60000)),
        roomHealthMaxStaleMs: Math.max(20000, normalizeInt(merged.roomHealthMaxStaleMs, 300000)),
        healthMetricsIntervalMs: Math.max(60000, normalizeInt(merged.healthMetricsIntervalMs, 300000)),
        disconnectFlushMs: Math.max(100, normalizeInt(merged.disconnectFlushMs, 250)),
...
const baseEntBotSettings = {
    ...
    roomHealthIntervalMs: envInt('ENT_BOT_ROOM_HEALTH_INTERVAL_MS', 60000),
    roomHealthMaxStaleMs: envInt('ENT_BOT_ROOM_HEALTH_MAX_STALE_MS', 300000),
    healthMetricsIntervalMs: envInt('ENT_BOT_HEALTH_METRICS_INTERVAL_MS', 300000),
    disconnectFlushMs: envInt('ENT_BOT_DISCONNECT_FLUSH_MS', 250),
